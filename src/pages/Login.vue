...
async function loginWithGoogle() {
    try {
        ...
        const credentials = await signInWithPopup(fireauth, new GoogleAuthProvider())
...
async function login() {
    try {
        console.log(email.value, password.value)
        const credentials = await signInWithEmailAndPassword(fireauth, email.value, password.value)
        authStore.profile = credentials.user
        await router.push('/')
    } catch (error: any) {
        switch (error.code) {

...
<template>
    ...
    <div class="player-wrapper">
        <button v-if="!playerStore.isLoading" @click="playerStore.setCurrentTrack(playerStore.currentTrack?.data.id! - 1)">
            <img src="/assets/foward.svg" style="transform: rotate(180deg);"/>
        </button>
        <button v-if="!playerStore.isLoading" @click="playerStore.isPlaying ? pause() : play()">
            <img :src="playerStore.isPlaying ? '/assets/pause.svg' : '/assets/play.svg'" />
        </button>
        <button v-if="!playerStore.isLoading" @click="playerStore.setCurrentTrack(playerStore.currentTrack?.data.id! + 1)">
            <img src="/assets/foward.svg"/>
        </button>
        <p>{{ formatDuration(playerStore.currentDuration) }} - {{ formatDuration(playerStore.duration) }}</p>
        <input
            type="range"
